class CreateShippingFoundation < ActiveRecord::Migration[8.0]
  ...
  def up
    add_checkout_shipping_snapshot
  ...
  def down
    ...
    remove_column :checkout_sessions, :shipping_address
  end

  private

  def add_checkout_shipping_snapshot
    add_column :checkout_sessions, :shipping_address, :jsonb, null: false, default: {}
  ...
  def create_provider_accounts
    ...
    add_index :shipping_provider_accounts,
              [:tenant_id, :store_id, :provider_key, :mode],
              unique: true,
              name: "idx_shipping_accounts_provider_unique"
    add_index :shipping_provider_accounts,
              [:id, :tenant_id, :store_id],
              unique: true,
              name: "idx_shipping_accounts_id_tenant_store"
    add_check_constraint :shipping_provider_accounts,
                         "provider_key ~ '^[a-z0-9][a-z0-9_.-]{1,63}$'",
                         name: "shipping_accounts_provider_key_check"
    add_check_constraint :shipping_provider_accounts,
                         "mode IN ('test', 'live')",
                         name: "shipping_accounts_mode_check"
    add_check_constraint :shipping_provider_accounts,
                         "status IN ('active', 'disabled')",
                         name: "shipping_accounts_status_check"
  end
  ...
  def create_rate_quotes
    ...
    add_index :shipping_rate_quotes,
              [:tenant_id, :store_id, :checkout_session_id, :request_digest, :service_code],
              unique: true,
              name: "idx_shipping_quotes_request_service"
    add_index :shipping_rate_quotes,
              [:id, :tenant_id, :store_id],
              unique: true,
              name: "idx_shipping_quotes_id_tenant_store"
    add_check_constraint :shipping_rate_quotes,
                         "currency = upper(currency) AND char_length(currency) = 3",
                         name: "shipping_quotes_currency_check"
    add_check_constraint :shipping_rate_quotes,
                         "amount_cents >= 0",
                         name: "shipping_quotes_amount_check"
  end
  ...
  def create_shipments
    ...
    add_index :shipments,
              ...
              unique: true,
              name: "idx_shipments_idempotency"
    add_index :shipments,
              [:shipping_provider_account_id, :provider_shipment_id],
              unique: true,
              where: "provider_shipment_id IS NOT NULL",
              name: "idx_shipments_provider_reference"
    add_index :shipments,
              [:id, :tenant_id, :store_id],
              unique: true,
              name: "idx_shipments_id_tenant_store"
    add_check_constraint :shipments,
                         "status IN ('pending', 'submitted', 'label_ready', 'in_transit', 'delivered', 'failed', 'cancelled')",
                         name: "shipments_status_check"
    add_check_constraint :shipments,
                         "currency = upper(currency) AND char_length(currency) = 3",
                         name: "shipments_currency_check"
    add_check_constraint :shipments,
                         "shipping_cost_cents >= 0",
                         name: "shipments_cost_check"
  end
  ...
  def create_shipment_events
    ...
    add_index :shipment_events,
              [:tenant_id, :store_id, :shipment_id, :occurred_at],
  ...
  def configure_event_immutability
    ...
    execute <<~SQL
      ...
      FOR EACH ROW EXECUTE FUNCTION crystell.prevent_shipment_event_mutation()
    SQL
  end
