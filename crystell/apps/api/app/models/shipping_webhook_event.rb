class ShippingWebhookEvent < ApplicationRecord
  belongs_to :tenant
  belongs_to :store
  belongs_to :shipping_provider_account
  belongs_to :shipment, optional: true

  validates :provider_event_id, :event_type, :payload_digest, :signature_digest, :raw_body_ciphertext, presence: true
  validates :status, inclusion: { in: %w[received processed ignored failed] }

  def raw_body
    Shipping::CredentialVault.decrypt(
      raw_body_ciphertext,
      purpose: "crystell:shipping-webhook-body:v1"
    )
  end

  def raw_body=(value)
    self.raw_body_ciphertext = Shipping::CredentialVault.encrypt(
      value.to_s,
      purpose: "crystell:shipping-webhook-body:v1"
    )
  end
end
