...
module Communications
  class CredentialVault
    ...
    def self.key
      encoded = ENV["COMMUNICATION_SECRETS_KEY"].to_s
      if encoded.present?
        decoded = Base64.strict_decode64(encoded)
        raise ConfigurationError, "COMMUNICATION_SECRETS_KEY must decode to 32 bytes" unless decoded.bytesize == KEY_BYTES

        return decoded
      end
      raise ConfigurationError, "COMMUNICATION_SECRETS_KEY is required in production" if Rails.env.production?

      Digest::SHA256.digest("#{Rails.application.secret_key_base}:crystell:communications")
    rescue ArgumentError
      raise ConfigurationError, "COMMUNICATION_SECRETS_KEY must be strict base64"
    end
