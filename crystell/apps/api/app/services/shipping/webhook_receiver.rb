...
module Shipping
  class WebhookReceiver
    ...
    def self.call(endpoint_id:, raw_body:, headers:, now: Time.current)
      resolution = Shipping::WebhookEndpointResolver.call(endpoint_id: endpoint_id)

      TenantSystemAccess.with(tenant_id: resolution.tenant_id) do
        account = ShippingProviderAccount.active.find_by!(
          id: resolution.shipping_provider_account_id,
          store_id: resolution.store_id
        )
        adapter = Shipping::AdapterRegistry.build(account)
        ...
        unless %w[processed ignored].include?(event.status)
          begin
            Shipping::WebhookEventProcessor.call(event: event, parsed: parsed, now: now)
    ...
    def self.find_or_create_event!(account:, parsed:, raw_body:, payload_digest:, signature_digest:, now:)
      existing = ShippingWebhookEvent.find_by(
      ...
      begin
        ApplicationRecord.transaction(requires_new: true) do
          created = ShippingWebhookEvent.create!(
