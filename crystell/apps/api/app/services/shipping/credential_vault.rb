...
module Shipping
  class CredentialVault
    ...
    def self.key
      encoded = ENV["SHIPPING_SECRETS_KEY"].to_s
      if encoded.present?
        decoded = Base64.strict_decode64(encoded)
        raise ConfigurationError, "SHIPPING_SECRETS_KEY must decode to 32 bytes" unless decoded.bytesize == KEY_BYTES

        return decoded
      end

      if Rails.env.production?
        raise ConfigurationError, "SHIPPING_SECRETS_KEY is required in production"
      end

      Digest::SHA256.digest("#{Rails.application.secret_key_base}:crystell:shipping-secrets")
    rescue ArgumentError
      raise ConfigurationError, "SHIPPING_SECRETS_KEY must be strict base64"
    end
